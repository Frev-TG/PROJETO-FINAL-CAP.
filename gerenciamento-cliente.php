...
if (isset($_GET["delete_id"])) {
    ...
    $sql_delete = "DELETE FROM cadastro_cliente WHERE id=?";
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["id"])) {
    ...
    $sql_update = "UPDATE cadastro_cliente SET nome=?, cpf=?, email=?, telefone=?, data_nascimento=? WHERE id=?";
...
// Consulta SQL para pegar os dados da tabela "cadastro_cliente"
$sql = "SELECT id, nome, cpf, email, telefone, data_nascimento FROM cadastro_cliente";
                    ...
                    <tbody>
                        ...
                        if ($result->num_rows > 0) {
                            while ($row = $result->fetch_assoc()) {
                                ...
                                echo "<td class='editable-cell' contenteditable='false' oninput='this.textContent = this.textContent.replace(/\\D/g, \'\')'><span contenteditable='true' onkeypress='return isNumberKey(event)' maxlength='11'>" . $row["cpf"] . "</span></td>";
                                echo "<td class='editable-cell' contenteditable='false'>" . $row["email"] . "</td>";
                                echo "<td class='editable-cell' contenteditable='false' oninput='this.textContent = this.textContent.replace(/\\D/g, \'\')'><span contenteditable='true' onkeypress='return isNumberKey(event)' maxlength='15'>" . $row["telefone"] . "</span></td>";
            ...
            btn.addEventListener('click', function() {
                ...
                this.addEventListener('click', function() {
                    ...
                    if (confirmacao) {
                        ...
                        const cpf = cells[1].querySelector('span').textContent.replace(/\D/g, ''); // Remover caracteres não numéricos
                        const email = cells[2].textContent;
                        const telefone = cells[3].querySelector('span').textContent.replace(/\D/g, ''); // Remover caracteres não numéricos
